...
class CustomTranslationFiles_AdminPageFramework_Form_View___Script_CollapsibleSection extends CustomTranslationFiles_AdminPageFramework_Form_View___Script_Base {
...
( function( $ ) {
    ...
    jQuery( document ).ready( function() {
        jQuery( this ).initializeCustomTranslationFiles_AdminPageFrameworkCollapsibleSections();
    });              
     ...
     * @since    3.7.0
     */
    jQuery( document ).bind( 'custom-translation-files_saved_widget', function( event, oWidget ){
        jQuery( oWidget ).initializeCustomTranslationFiles_AdminPageFrameworkCollapsibleSections();
    });    
    
    $.fn.initializeCustomTranslationFiles_AdminPageFrameworkCollapsibleSections = function() {
        
        // Expand collapsible sections that are set not to collapse by default 
        jQuery( this ).find( '.custom-translation-files-collapsible-sections-title[data-is_collapsed=\"0\"]' )
            .next( '.custom-translation-files-collapsible-sections-content' )
            .slideDown( 'fast' );
        jQuery( this ).find( '.custom-translation-files-collapsible-section-title[data-is_collapsed=\"0\"]' )
            .closest( '.custom-translation-files-section-table' )
            .find( 'tbody' )
            .slideDown( 'fast' );
            
        // Hide collapsible sections of 'section' containers as they are somehow do not get collapsed by default.
        jQuery( this ).find( '.custom-translation-files-collapsible-section-title[data-is_collapsed=\"1\"]' )
            .closest( '.custom-translation-files-section-table' )
            .find( 'tbody' )
            .hide();
        
        // Bind the click event to the title element.
        jQuery( this ).find( '.custom-translation-files-collapsible-sections-title, .custom-translation-files-collapsible-section-title' ).enableCustomTranslationFiles_AdminPageFrameworkCollapsibleButton();
        
        // Insert the toggle all button.
        jQuery( this ).find( '.custom-translation-files-collapsible-title[data-toggle_all_button!=\"0\"]' ).each( function(){
            
            var _oThis        = jQuery( this ); // to access from inside the below each() method.
            var _bForSections = jQuery( this ).hasClass( 'custom-translation-files-collapsible-sections-title' );   // or for the 'section' container.
            ...
            jQuery.each( _aPositions, function( iIndex, _sPosition ) {
                ...
                var _sLeftOrRight = -1 !== jQuery.inArray( _sPosition, [ 'top-right', 'bottom-right', '0' ] )   // if found
                    ? 'right'   // default
                    : 'left';            
                _oButton.find( '.custom-translation-files-collapsible-toggle-all-button' ).css( 'float', _sLeftOrRight );
                ...
                if ( _bForSections ) {
                    var _oTargetElement = 'before' === _sTopOrBottom
                        ? _oThis
                        : _oThis.next( '.custom-translation-files-collapsible-content' );
                        _oTargetElement[ _sTopOrBottom ]( _oButton );
                } else {    // for 'section' containers
                    _oThis.closest( '.custom-translation-files-section' )[ _sTopOrBottom ]( _oButton );
                }                
                ...
                _oButton.click( function(){                  
                    ...
                    var _oButtons = _bForSections
                        ? jQuery( this ).closest( '.custom-translation-files-sectionset' ).siblings().andSelf().find( '> .custom-translation-files-collapsible-toggle-all-button-container' )
                        : jQuery( this ).siblings( '.custom-translation-files-collapsible-toggle-all-button-container' ).andSelf();
                    _oButtons.toggleClass( 'flipped' );
                    if ( _bForSections ) {
                        _oButton.parent().parent().children().children( '* > .custom-translation-files-collapsible-title' ).each( function() {
                            jQuery( this ).trigger( 'click', [ 'by_toggle_all_button' ] );
                        } );
                    } else {
                        _oButton.closest( '.custom-translation-files-sections' ).children( '.custom-translation-files-section' ).children( '.custom-translation-files-section-table' ).children( 'caption' ).children( '.custom-translation-files-collapsible-title' ).each( function() {
                            jQuery( this ).trigger( 'click', [ 'by_toggle_all_button' ] );
                        } );
                    }
     ...
     */
    $.fn.enableCustomTranslationFiles_AdminPageFrameworkCollapsibleButton = function() {
        ...
        jQuery( this ).click( function( event, sContext ){
            ...
            var _sContainerType = jQuery( this ).hasClass( 'custom-translation-files-collapsible-sections-title' )
                ? 'sections'
                : 'section';
            var _oTargetContent = 'sections' === _sContainerType
                ? jQuery( this ).next( '.custom-translation-files-collapsible-content' ).first()
            ...
            if ( 'expand' === _sAction && _oThis.data( 'collapse_others_on_expand' ) ) {
                _oThis.parent().parent().children().children( '* > .custom-translation-files-collapsible-content' ).not( _oTargetContent ).slideUp( 'fast', function() {
                    jQuery( this ).prev( '.custom-translation-files-collapsible-title' ).addClass( 'collapsed' );
                });
            }
...
JAVASCRIPTS;
    ...
    static private function _getToggleAllButtonHTML($oMsg) {
        ...
        return "<div class='custom-translation-files-collapsible-toggle-all-button-container'>" . "<span class='custom-translation-files-collapsible-toggle-all-button button " . $_sDashIconSort . "'" . " title='" . esc_attr($_sLabelToggleAllSections) . "'>" . $_sText . "</span>" . "</div>";
    }
}
